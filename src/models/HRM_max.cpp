...
void HRM_max::parametersToFlatVector(	double*    g,
										double***  U,
										double***  V,
										action_t   action)
{
	...
	if (ind != NW) {
		printf("Got incorrect index (%d != %d) at line %d of HRM_max.cpp\n", ind, NW, __LINE__);
		exit(1);
	}
}

double HRM_max::prediction(int user, int item_prev, int item)
{
	double pred = 0;
	for (int k = 0; k < K; k ++) {
		pred += max(U[user][k], V[item_prev][k]) * V[item][k]; // max pooling
	}
	return pred;
}

void HRM_max::train(int iterations, double learn_rate)
{
	printf("%s", ("\n<<< " + toString() + " >>>\n\n").c_str());
...
void HRM_max::oneIteration(double learn_rate)
{
	// working memory
...
void HRM_max::updateFactors(int user, int prev_item, int item, double learn_rate)
{
	...
	for (int k = 0; k < K; k ++) {
		V_hybrid[k] = max(U[user][k], V[prev_item][k]);
	}
...
string HRM_max::toString()
{
	char str[100];
	sprintf(str, "HRM_MaxPooling_BPR_K_%d_lambda_%f", K, lambda);
	return str;
}
