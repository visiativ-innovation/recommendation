...
void TransRec::parametersToFlatVector(	double*    g,
										double**   beta_item,
										double***  H,
										double***  R,
										double**   r,
...
double TransRec::prediction(int user, int item_prev, int item)
{
	double pred = beta_item[item];
	for (int k = 0; k < K; k ++) {
		pred += square(H[item_prev][k] + r[k] + R[user][k] - H[item][k]);
	}
	return -pred;
}

void TransRec::train(int iterations, double learn_rate)
{
	printf("%s", ("\n<<< " + toString() + " >>>\n\n").c_str());
...
void TransRec::oneIteration(double learn_rate)
{
	// working memory
	...
	for (int i = 0; i < num_pos_events; i ++) {
		...
		// } while (neg_item_id == item_id);  // Replace above line of code when data is Google Local
...
void TransRec::updateFactors(int user, int x, int y, int yn, double learn_rate)
{
	...
	for (int k = 0; k < K; k ++) {
		H[x][k]    += learn_rate * (deri * 2 * (-v_x_y[k] + v_x_yn[k]) - lambda * H[x][k]);
		H[yn][k]   += learn_rate * (deri * 2 * (-v_x_yn[k]) - lambda * H[yn][k]);
		H[y][k]    += learn_rate * (deri * 2 * (v_x_y[k]) - lambda * H[y][k]);
		r[k]       += learn_rate * (deri * 2 * (-v_x_y[k] + v_x_yn[k]) - lambda * r[k]);
		R[user][k] += learn_rate * (deri * 2 * (-v_x_y[k] + v_x_yn[k]) - relation_reg * R[user][k]);
	}
...
{
	char str[100];
	sprintf(str, "TransRec__K_%d_lambda_%f_relationReg_%f_biasReg_%f", K, lambda, relation_reg, bias_reg);
	return str;
}
